...
func TestLogCustomSaveMoneyEvents(t *testing.T) {
	...
	// Thus demonstrating that the CustomSaveMoneyEvents code generates the same output
	// as the SaveMoneyEvents code, but using less HEAP allocation - as demonstrated in
	// the Benchmark code : BenchmarkLogCustomSave()
	...
	///////////////////////////////////
	// Capture custom save money events

	var newLength1, newLength2, newLength3 int
	isMinimalAllocations = true // use new CustomSaveMoneyEvent1() code, for minimum memory allocations

	// 1st Event is like the first one in Middleware()
	CustomSaveMoneyEvent1(ctx, "http request received",
		req, 0, 0,
		&start, nil)
	...
	// 2nd event is 'similar in length' to one in createReverseProxy()
	CustomSaveMoneyEvent2(ctx, "proxying request", INFO,
		req2, 0, 0,
	...
	CustomSaveMoneyEvent3(ctx, "http request completed",
		req2, 200, 4,
		nil, &end)
